...
static void visa(signal_typ_t typ, const char *text, int ms, uint32_t farg, bool hors)
{
    ...
    if (typ >= SIGNAL_SMS && typ <= SIGNAL_BACKUP) {
        ...
        ansikte_tillfalligt(typ == SIGNAL_BACKUP ? UTTRYCK_OROLIG : UTTRYCK_NYFIKEN, ms);
        if (typ == SIGNAL_SMS || typ == SIGNAL_TEAMS) {
            ansikte_ikon(typ == SIGNAL_SMS ? 2 : 3, farg, ms);
            ansikte_titta(.8f, -.7f);
        } else ansikte_titta(0, .8f);
        ansikte_sag(text, ms);
