...
static void rita_oga(lv_layer_t *l, const oga_t *o, float cx, float cy, bool hoger, float oppen_extra, float snurr)
{
    ...
    bool tecken = signal_kvar > 0 && !sover &&
        (signal_typ == 1 || signal_typ == 2 || signal_typ == 3 || (signal_typ == 5 && klar_logga && !hoger));
    if (tecken && h > 30) {
        if (signal_typ == 1) symbol(l, 1, cx, cy, fminf(w*.32f,h*.35f), FARG_PUPILL);
...
static void rutan(lv_area_t *ut)
{
    ...
    if (signal_kvar > 0) {
        ut->y1 -= 20; /* klart-skutt */
        area_satt(&a, 20, 6, ANSIKTE_BREDD - 20, 85);
        area_utvidga(ut, &a);
    }
    if (leka_kvar_ms > 0) {
