...
def starta_brevlada():
    """Vid en omstart kan den gamla processen hålla porten några sekunder.
    Ge aldrig upp: utan brevlådan når inga notiser, krokar eller frågor kortet."""
    def lyssna():
        klagat = False
        while True:
            try:
                ...
                if not klagat:
                    logg(f"brevlådan kunde inte öppnas på {BREVLADA_PORT}: {fel}, försöker igen")
                    klagat = True
                time.sleep(3)
        logg(f"brevlådan lyssnar på http://127.0.0.1:{BREVLADA_PORT}/")
        server.serve_forever()
    threading.Thread(target=lyssna, daemon=True).start()
